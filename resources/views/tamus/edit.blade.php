    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <input type="text" name="id_tamu" class="form-control" placeholder="ID tamu" value="{{ $tamu->id_tamu }}" disabled>
                @error('id_tamu')
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <input type="date" name="tanggal" class="form-control" placeholder="tanggal tamu" value="{{ $tamu->tanggal }}">
                @error('tanggal')
                <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <input type="text" name="nama_alamat" class="form-control" placeholder="nama_alamat" value="{{ $tamu->nama_alamat }}">
                @error('name')
                <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <select name="nama" id="nama" class="form-select">
                    ...
                    <option value="{{ $item->id }}" {{ ($item->id==$tamu->nama)? 'selected': ''}}>{{ $item->name }}</option>
                    @endforeach
                </select>
                @error('id_nama')
                <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
      ...
      <div class="col-xs-12 col-sm-12 col-md-12 mt-3">
        <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody id="detail">
                ...
                <tr>
                    <td>
                        <input type="hidden" name="acaraId{{$no}}" class="form-control" value="{{$item->id_acara}}">
                        <span>{{$no}}</span>
                    </td>
                    <td>
                        <input type="text" name="acara{{$no}}" class="form-control" value="{{$item->getacara->acara}}">
                    </td>
                    <td>
                        <input type="text" name="tempat{{$no}}" class="form-control" value="{{$item->tempat}}">
                    </td>
                    <td>
                        <input type="text" name="panitia{{$no}}" class="form-control" value="{{$item->panitia}}">
                    </td>
                    <td>
                        <input type="text" name="tamu{{$no}}" class="form-control" value="{{$item->tamu}}" >
                    </td>
                    <td>
